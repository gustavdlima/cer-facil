  ...
  const renderProfessionalCard = (prof: any) => {
    ...
    return (
      ...
      >
        ...
        >
          <span
            className={`font-bold text-sm transition-colors ${isOpen ? "text-[var(--cor-bg-1)]" : "text-slate-800"}`}
          >
            <span
              className={`inline-block w-2.5 h-2.5 rounded-full mr-3 bg-[var(--cor-bg-1)]`}
            />
            {prof.professional}
          </span>
          <ChevronDown
              ...
              {filterOptions.map((option) => (
                ...
                >
                  <span className="flex items-center justify-center gap-2">
                    ...
                    {option.label}
                    {activeFilters.includes(option.id) && (
                      <X size={14} className="inline ml-2" />
                    )}
                  </span>
                </button>
              ))}
